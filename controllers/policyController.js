...
const reviewClaimBySurveyor = async (req, res) => {
  try {
    ...
    // ✅ Construct absolute image paths correctly
    const uploadsDir = path.join(__dirname, "uploads"); // Define correct uploads directory
    ...
    console.log(`🔹 Damage Image Path: ${damageImagePath}`);

    // ✅ Ensure images exist before sending them
    ...
    // ✅ Send images as FormData to Flask
    ...
    try {
      ...
      const { damagePercentage } = response.data;

      // ✅ Store damage percentage inside MongoDB
